        ...
        <nav className={navbarClasses}>
            <div className="container mx-auto px-4 md:px-8 flex justify-between items-center">
                ...
                <Link to="/" className="flex-none items-center hidden md:flex">
                    <img src={logo} alt="WearStylewithImtisall" className="h-14 md:h-18 object-contain" />
                </Link>

                {/* Mobile Logo (Center on mobile only) */}
                <Link to="/" className="md:hidden flex-1 flex justify-center items-center">
                    <img src={logo} alt="WearStylewithImtisall" className="h-14 object-contain" />
                </Link>
                    ...
                    {categories.map(cat => (
                        cat.children && cat.children.length > 0 ? (
                            <div key={cat._id} className="relative group/dropdown py-2">
                                <div className={`${linkClasses} flex items-center gap-1 whitespace-nowrap cursor-default`}>
                                    {cat.name}
